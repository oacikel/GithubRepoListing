...
class RepoDetailFragmentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun setup(arguments: Bundle, view: View, toolbar: Toolbar?) {
        ...
        val drawbleFavourited = view.context!!.resources.getDrawable(R.drawable.icon_favorite,view.context!!.theme)
        val drawbleNotFavourited = view.context!!.resources.getDrawable(R.drawable.icon_favourite_empty,view.context!!.theme)

        if (prefRepository.getFavouriteIds()?.contains(selectedRepo.id.toString()) == true) {
            toolbar?.menu?.get(0)?.icon=drawbleFavourited
        } else {
            toolbar?.menu?.get(0)?.icon=drawbleNotFavourited
        }
    }
